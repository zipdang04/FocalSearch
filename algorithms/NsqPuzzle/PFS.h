...
#include "testlib.h"
class ProbabilisticFocalSearch: public PuzzleAlgorithm {
	private:
		struct SearchState{
			BoardState board;
			int f, g, h;
		...
		void execute() override {
			...
			while (not focalList.empty()) {
				...
				SearchState node = (rnd.next(1.0) < pFocal) 
					? *focalList.begin()
					: *openList.begin();
	...
	public:
		ProbabilisticFocalSearch(BoardState initialState, std::function<int(BoardState)> hFunc, std::function<int(BoardState)> focalFunc, double pFocal, double eps = 1.1)
			: PuzzleAlgorithm(initialState), hFunc(hFunc), focalFunc(focalFunc), pFocal(pFocal), eps(eps) {
			ensure(0 <= pFocal and pFocal <= 1);
			algoName = "Probabilistic Focal Search with pFocal = " + toString(pFocal);
		}
};
